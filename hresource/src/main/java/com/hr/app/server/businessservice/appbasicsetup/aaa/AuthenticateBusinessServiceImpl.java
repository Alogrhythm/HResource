package com.hr.app.server.businessservice.appbasicsetup.aaa;
import com.hr.app.config.appSetup.model.AppConfiguration;

import com.hr.app.util.IpAddress;

import com.hr.app.bean.TokenCredentialBean;
...
public class AuthenticateBusinessServiceImpl implements AuthenticateBusinessService {
	...
	public HttpEntity<ResponseBean> authenticate(@RequestBody LoginCredentials loginBean, HttpServletRequest request, HttpServletResponse response, HttpSession session) {
		...
		try {
			...
			if (userBean != null) {
				saveDadaInSession(userBean, session, request, response);
	...
	private void saveSessionData(HttpSession session, HttpServletRequest request, UserBean userBean) throws Exception {
		...
		loginSessionDataRepository.saveSessionData(session.getAttribute("userId").toString(), session.getAttribute("userId").toString(), DATA_TYPE.NUMBER.value, null, null, userBean.getProperties().get("userAccessCode").toString(),
	...
	private void saveDadaInSession(UserBean userBean, HttpSession session, HttpServletRequest request, HttpServletResponse response) throws Exception {
