Ext.define('Hresource.hresource.web.com.hr.controller.humanresourceboundedcontext.payroll.CostToCompanyMainController', {
     extend: 'Hresource.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CostToCompanyMainController',
     ...
     empIdLoad: function() {
          ...
          Ext.Ajax.request({
               url: 'secure/EmployeeInfo/findAll',
               method: 'GET',
               async: false,
               sender: scope,
               jsonData: {},
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CostToCompanyTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#CostToCompanyForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#CostToCompanyGrid');
               var tree = this.view.down('#CostToCompanyTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#CostToCompanyForm');
          var grid = this.view.down('#CostToCompanyGrid');
          var tree = this.view.down('#CostToCompanyTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#CostToCompanyGrid');
               ...
               var formPanel = this.getView().up().down('#CostToCompanyForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#CostToCompanyTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CostToCompanyForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CostToCompanyGrid').store;
          gridStore.removeAll();
