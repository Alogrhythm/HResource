Ext.define('Hresource.hresource.web.com.hr.controller.humanresourceboundedcontext.employee.DepartmentTypeMainController', {
     extend: 'Hresource.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.DepartmentTypeMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#DepartmentTypeTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#DepartmentTypeForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#DepartmentTypeGrid');
               var tree = this.view.down('#DepartmentTypeTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#DepartmentTypeForm');
          var grid = this.view.down('#DepartmentTypeGrid');
          var tree = this.view.down('#DepartmentTypeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#DepartmentTypeGrid');
               ...
               var formPanel = this.getView().up().down('#DepartmentTypeForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#DepartmentTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#DepartmentTypeForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#DepartmentTypeGrid').store;
          gridStore.removeAll();
