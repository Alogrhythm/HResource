Ext.define('Hresource.hresource.web.com.hr.controller.humanresourceboundedcontext.employee.EmployeeInfoMainController', {
     extend: 'Hresource.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.EmployeeInfoMainController',
     /*********************Main Controller Functions*********************************/
     init: function() {
          var but = this.view.down('#resetFormButton');
          but.up('form').reset();
          this.deptTypeCodeLoad();
          this.designationTypeCodeLoad();
          this.coreContactsLoad();
          this.jobTypeCodeLoad();
     ...
     deptTypeCodeLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               success: function(response, scope) {
                    var compRef = scope.sender.down('#deptTypeCode');
                    scope.sender.controller.assignAllComboData(scope.sender, 'deptTypeCode', response.responseText);
     ...
     designationTypeCodeLoad: function() {
          var scope = this.getView();
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#designationTypeCode');
                    scope.sender.controller.assignAllComboData(scope.sender, 'designationTypeCode', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#designationTypeCode');
     ...
     coreContactsLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#coreContacts');
                    scope.sender.controller.assignAllComboData(scope.sender, 'coreContacts', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#coreContacts');
     ...
     jobTypeCodeLoad: function() {
          ...
          Ext.Ajax.request({
               ...
               jsonData: {},
               success: function(response, scope) {
                    var compRef = scope.sender.down('#jobTypeCode');
                    scope.sender.controller.assignAllComboData(scope.sender, 'jobTypeCode', response.responseText);
                    scope.sender.controller.addCommunicationLog(response, true, compRef);
               },
               failure: function(response, scope) {
                    var compRef = scope.sender.down('#jobTypeCode');
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#EmployeeInfoTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#EmployeeInfoForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#EmployeeInfoGrid');
               var tree = this.view.down('#EmployeeInfoTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#EmployeeInfoForm');
          var grid = this.view.down('#EmployeeInfoGrid');
          var tree = this.view.down('#EmployeeInfoTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#EmployeeInfoGrid');
               ...
               var formPanel = this.getView().up().down('#EmployeeInfoForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#EmployeeInfoTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#EmployeeInfoForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#EmployeeInfoGrid').store;
          gridStore.removeAll();
