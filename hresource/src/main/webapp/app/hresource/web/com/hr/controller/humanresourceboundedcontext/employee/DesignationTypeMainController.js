Ext.define('Hresource.hresource.web.com.hr.controller.humanresourceboundedcontext.employee.DesignationTypeMainController', {
     extend: 'Hresource.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.DesignationTypeMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#DesignationTypeTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#DesignationTypeForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#DesignationTypeGrid');
               var tree = this.view.down('#DesignationTypeTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#DesignationTypeForm');
          var grid = this.view.down('#DesignationTypeGrid');
          var tree = this.view.down('#DesignationTypeTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#DesignationTypeGrid');
               ...
               var formPanel = this.getView().up().down('#DesignationTypeForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var data = this.findRecordById(this.view.restURL, jsonData);
          var treePanel = this.view.up().up().down('#DesignationTypeTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#DesignationTypeForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#DesignationTypeGrid').store;
          gridStore.removeAll();
